...
pub mod pallet {
	...
	parameter_types! {
		...
		pub const MaxLength : u32 = 100;
		/// The minimum length of a hash.
		pub const MinLength : u32 = 1;
	}
	...
	pub enum Event<T: Config> {
		...
		/// Event emitted when a proof has been transferred. [who, dist, claim]
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_claim(origin: OriginFor<T>, proof: Vec<u8>) -> DispatchResult {
			// Check that the length of proof can not illegal
			ensure!(proof.len() <= T::MaxLength::get() as usize, Error::<T>::ProofLengthIllegal);
			ensure!(proof.len() >= T::MinLength::get() as usize, Error::<T>::ProofLengthIllegal);
		...
		pub fn transfer_claim(
			...
			// get the actually owner of the proof
